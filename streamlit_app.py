...
if len(st.session_state.df) == 0:
    ...
    display_df["이미지 URL"] = display_df["이미지 URL"].fillna("").apply(
        lambda x: "📷 있음" if x.strip() != "" else ""
    )
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "이미지 URL": st.column_config.TextColumn(
                "이미지",
                help="첨부 이미지 여부",
            ),
        },
    ...
    if not edited_df["상태"].equals(st.session_state.df["상태"]) or not edited_df["우선순위"].equals(st.session_state.df["우선순위"]):
        changed_mask = (
        ...
        for idx in edited_df[changed_mask].index:
            ...
            row["상태"] = edited_df.loc[idx, "상태"]
            row["우선순위"] = edited_df.loc[idx, "우선순위"]
            save_ticket(row.to_dict())
            st.session_state.df.loc[idx, "상태"] = edited_df.loc[idx, "상태"]
            st.session_state.df.loc[idx, "우선순위"] = edited_df.loc[idx, "우선순위"]
        st.rerun()
